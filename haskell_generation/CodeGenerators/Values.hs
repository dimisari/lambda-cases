...
one_arg_function_applications_g = (
  ...
  ) :: ValueType -> OneArgFunctionApplications -> Stateful Haskell

-- MultiplicationFactor

multiplication_factor_g = ( \vt -> \case
  OneArgApplicationsMF oaas -> one_arg_function_applications_g vt oaas
...
subtraction_factor_g = ( \vt -> \case
  MultiplicationSF m -> multiplication_g vt m
  OneArgApplicationsSF oaas -> one_arg_function_applications_g vt oaas
...
no_abstractions_value_1_g = ( \vt -> \case
  Sub sub -> subtraction_g vt sub
  Mul mul -> multiplication_g vt mul
...
many_args_app_arg_value_g = (
  ...
  return $ abstractions_g bts2 (Abstractions as) ++ nav1_g
  ) :: ValueType -> ManyArgsAppArgValue -> Stateful Haskell
...
many_args_application_g = ( \vt(ManyArgsApplication maaavs vn) -> 
  ...
  help_g = \case 
    ...
    :: ManyArgsAppArgValue -> Stateful Haskell
  in
...
no_abstractions_value_g = ( \vt -> \case
  ManyArgsApp maa -> many_args_application_g vt maa
...
value_g = (
  \(AbstractionTypesAndResultType bts bt) ->
  \(Value (Abstractions as) nav) ->
  let
  ( bts1, bts2 ) = splitAt (length as) bts
  vt = AbstractionTypesAndResultType bts2 bt
  in
  no_abstractions_value_g vt nav >>= \nav_g ->
  return $ abstractions_g bts1 (Abstractions as) ++ nav_g
