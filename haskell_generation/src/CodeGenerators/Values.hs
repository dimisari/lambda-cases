  ...
  generate = \(Cases cases maybe_def_case) val_type ->
    ...
    type_map_get type_name >>= \case
      ...
      OrType _ or_cases -> 
        ...
        check_lovns lovns type_name or_t_cases_names has_default >>= \val_names ->
        or_type_cases_g
...
last_or_type_vn_g = ( \val_name or_t_cs val_type -> case elem val_name or_t_cs of
  True -> case or_t_cs == [ val_name ] of 
    ...
    False -> throwE $ cases_not_covered_err (filter (/= val_name) or_t_cs)
  False -> val_n_ins_and_ret_hs val_name val_type
  ) :: ValueName -> [ ValueName ] -> ValType -> Stateful Haskell

not_last_or_type_case_g = (
  \(InAndOutTs in_t out_t) (or_t_cs, hs_so_far) (OrTypeCase val_name val_expr) ->
  get_ind_lev >>= \ind_lev ->
  ...
  ) :: FuncType -> ([ ValueName ], Haskell) -> OrTypeCase ->
       Stateful ([ ValueName ], Haskell)

last_or_type_case_g = (
  \(OrTypeCase val_name val_expr) (InAndOutTs in_t out_t) or_t_cs ->
  get_ind_lev >>= \ind_lev ->
  last_or_type_vn_g val_name or_t_cs in_t >>= \val_name_hs ->
  generate val_expr out_t >>= \val_expr_hs ->
  return $ indent ind_lev ++ val_name_hs ++ " -> " ++ val_expr_hs
  ) :: OrTypeCase -> FuncType -> [ ValueName ] ->
       Stateful Haskell
...
or_type_cases_with_default_g = (
  ...
  ) :: [ OrTypeCase ] -> DefaultCase -> [ ValueName ] ->
       FuncType -> Stateful Haskell
...
check_lovns = ( \lovns or_t_name or_t_cases_names has_default ->
